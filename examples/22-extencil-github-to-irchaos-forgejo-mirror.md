# Mirror GitHub User to a Custom Forgejo Instance: extencil and IRChaos
...
IRChaos is a trusted DFIR-focused community, and `extencil` has been invited to
...
| `IRCHAOS_TOKEN` | Yes | Creates and pushes repositories under `extencil` on `git.irchaos.club`. |
...
tokens = [{ env = "IRCHAOS_TOKEN", name = "irchaos-destination", username = "extencil" }]
visibility = "mirror"
push_mode = "portable-mirror"
```
...
export IRCHAOS_TOKEN="paste_irchaos_forgejo_token_here"
...
portable mirror refs to the IRChaos Forgejo instance.
